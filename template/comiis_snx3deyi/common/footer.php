...
<!--{if empty($topic) || ($topic[usefooter])}-->
	...
	<!--{if $comiis_ff==1}-->
		...
		<div id="ft" class="comiis_footer wp">
			<div class="comiis_footertop"><!--{loop $_G['setting']['footernavs'] $nav}--><!--{if $nav['available'] && ($nav['type'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1)) ||
						!$nav['type'] && ($nav['id'] == 'stat' && $_G['group']['allowstatdata'] || $nav['id'] == 'report' && $_G['uid'] || $nav['id'] == 'archiver' || $nav['id'] == 'mobile' || $nav['id'] == 'darkroom'))}-->$nav[code]|<!--{/if}--><!--{/loop}--> </div>
	<div class="comiis_Copyright">
	&copy; 2001-2013 <a href="$_G['setting']['siteurl']" target="_blank">$_G['setting']['sitename']</a>($_G['setting']['siteurl']) 版权所有 All Rights Reserved.<!--{if $_G['setting']['icp']}-->( <a href="http://www.miitbeian.gov.cn/" target="_blank">$_G['setting']['icp']</a> )<!--{/if}-->  <!--{if $_G['setting']['site_qq']}--><a href="http://wpa.qq.com/msgrd?V=3&Uin=$_G['setting']['site_qq']&Site=$_G['setting']['bbname']&Menu=yes&from=discuz" target="_blank" title="QQ"><img src="{IMGDIR}/site_qq.jpg" alt="QQ" /></a><!--{/if}-->
